...
class Bans {
	...
	function BanCheck($ip, $board = '', $force_display = false, $javascript_stub = false) {
		global $tc_db;
		...
		if(count($bans) > 0){
			...
			echo $this->DisplayBannedMessage($bans, '', $javascript_stub);
			die();
		}

		if ($force_display) {
			/* Instructed to display a page whether banned or not, so we will inform them today is their rucky day */
			echo '<title>'._gettext('YOU ARE NOT BANNED!').'</title><div align="center"><img src="'. KU_WEBFOLDER .'images/youarenotbanned.jpg"><br /><br />'._gettext('Unable to find record of your IP being banned.').'</div>';
		} else {
			return true;
		}
	}
	...
	function BanUser($ip, $modname, $globalban, $duration, $boards, $reason, $staffnote, $appealat=0, $type=0, $allowread=1, $proxyban=false) {
		...
		if ($duration>0) {
			$ban_until = time() + KU_ADDTIME + $duration;
		} else {
			$ban_until = '0';
		}
	...
	function DisplayBannedMessage($bans, $board='', $javascript_stub = false) {
		...
		setcookie_strict('tc_previousip', KU_REMOTE_ADDR, (time() + KU_ADDTIME + 604800), KU_BOARDSFOLDER);
	...
	function UpdateHtaccess() {
		...
		if ($htaccess_contents_bans_iplist!='') {
			...
			$htaccess_contents_bans_end = "RewriteRule !^(banned.php|images/youarebanned.jpg|favicon.ico|css/banned.css)$ " . KU_BOARDSFOLDER . "banned.php [L]\n</IfModule>";
		} else {
