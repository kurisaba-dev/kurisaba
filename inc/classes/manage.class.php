	...
	function viewthread() {
		...
		foreach ($results as $line) {
			...
			if ($thread == "0") {
				...
				$view = $deleted ? "[<a href=\"?action=restorethread&board=$board&thread=$id\">". _gettext('Undelete') ."]</a>" : "";
			}
			if ($name == "") {
		...
		if (isset($_GET['updateboard']) && isset($_POST['order']) && isset($_POST['maxpages']) && isset($_POST['maxage']) && isset($_POST['messagelength'])) {
			...
			if (count($resultsboard) > 0) {
				foreach ($resultsboard as $lineboard) {
					...
					$tpl_page .= '<label for="country_restrict">Restricted countries:</label>
					<input type="text" name="country_restrict" value="'.$lineboard['country_restrict'].'" />
					<div class="desc">ISO 3166-1 alpha-2 codes of countries, where this board should not be available, separated by commas. Example: RU,US,GB</div><br />';
					...
					/* Enable feed */
					$tpl_page .= '<label for="enablefeed">'. _gettext('Enable board in feed') .':</label>
					<input type="checkbox" name="enablefeed"';
					if ($lineboard['enablefeed'] == '1') {
						$tpl_page .= ' checked';
					}
					$tpl_page .= ' />
					<div class="desc">'. _gettext('If set to yes, this board will be shown in feed.') .' '. _gettext('Default') .': <strong>'. _gettext('Yes') .'</strong></div><br />';
