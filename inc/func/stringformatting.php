 ...
 */
function formatLongMessage($message, $board, $threadid, $page, $msgid) {
	$output = '';
	if ((strlen($message) > KU_CUTMSGLENGTH /*|| count(explode('<br />', $message)) > 15*/) && $page && KU_CUTPOSTS) {
		$message_exploded = explode('<br />', $message);
		$message_shortened = '';
		for ($i = 0; $i <= 14; $i++) {
			if (isset($message_exploded[$i])) {
				if (strlen($message_shortened . $message_exploded[$i]) > KU_CUTMSGLENGTH) break;
		...
		'	' . sprintf(_gettext('Message too long. Click %shere%s to view the full text.'), '<a href="' . KU_BOARDSFOLDER . $board . '/res/' . $threadid . '.html#' . $msgid . '" onclick="expand_post(this,\''.$board.'\','.$threadid.','.$msgid.'); return false;">', '</a>') . "\n" .
		'</div>' . "\n";
	} else {
		$output .= $message . "\n";
	}
