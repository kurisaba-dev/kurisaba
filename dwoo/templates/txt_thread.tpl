{foreach name=pst key=postkey item=post from=$posts}
		...
		<h3>
			...
			{if not $post.stickied && $post.parentid eq 0 && (($board.maxage > 0 && ($post + ($board.maxage * 3600)) < (time() + %KU_ADDTIME + 7200 ) ) || ($post.deleted_timestamp > 0 && $post.deleted_timestamp <= (time() + %KU_ADDTIME + 7200)))}
				<span class="oldpost">
