...
{
	//function hide_unhide_by_selector(sel, hidden)
	//{
	...
	if (hiders.length > 0)
	{
		hiders.forEach((h) =>
		{
			...
			{
				h.hidden = false;
				//hide_unhide_by_selector(".hider.bypostid."+id, true);
				//hide_unhide_by_selector(".hider.byparentid."+id, true);
				//hide_unhide_by_selector(".hider.byauthor."+id, true);
				//hide_unhide_by_selector(".hider.byword."+id, true);
			}
	...
	hiddenposts.forEach((post_to_hide) =>
	{
		...
		document.querySelectorAll('#reply' + p).forEach((post) =>
		{
			...
			if(post_b == b && !post.classList.contains("hiddenbypostid"))
			{
				...
				patch.innerHTML = "Пост "+p+" скрыт по номеру. Показать вновь: <a class=\"extrabtns\" href=\"#\" onclick=\"unhidepost_num('"+b+"',"+p+");return false;\" title=\"Показать этот пост\"><img src=\"/css/icons/blank.gif\" border=\"0\" class=\"unhidethread spritebtn\" alt=\"Показать этот пост\" /></a>";
				post.after(patch);
			}
		});
	});
					...
					if (p != "")
					{
						document.querySelectorAll('#reply' + p).forEach((post) =>
						{
							...
							if(post_b == b && !post.classList.contains("hiddenbythreadid"))
							{
								...
								patch.classList.add("id_" + p + "_" + b);
								patch.innerHTML = "Тред "+p+" скрыт по номеру. Показать вновь: <a class=\"extrabtns\" href=\"#\" onclick=\"unhidethread_num('"+b+"',"+p+");return false;\" title=\"Показать этот тред\"><img src=\"/css/icons/blank.gif\" border=\"0\" class=\"unhidethread spritebtn\" alt=\"Показать этот тред\" /></a>";
								post.after(patch);
							}
						});
						if (typeof(isfeed) != "undefined" && isfeed)
						{
							document.querySelectorAll('.reply_parent' + p).forEach((post) =>
							{
								...
								if(post_b == b && !post.classList.contains("hiddenbyparentid"))
								{
									...
									patch.innerHTML = "Пост "+a+" скрыт, так как тред "+p+" скрыт по номеру. Показать вновь: <a class=\"extrabtns\" href=\"#\" onclick=\"unhidethread_num('"+b+"',"+p+");return false;\" title=\"Показать этот тред\"><img src=\"/css/icons/blank.gif\" border=\"0\" class=\"unhidethread spritebtn\" alt=\"Показать этот тред\" /></a>";
									post.after(patch);
								}
							});
						}
	...
	process_posts.forEach((post) =>
	{
		...
		else
		{
			if (in_array(author, hiddenauthors))
			{
				...
				patch.classList.add("id_" + p + "_" + b);
				patch.innerHTML = "Пост " + p + ", написанный " + author + ", скрыт по автору. Показать его посты вновь: <a class=\"extrabtns\" href=\"#\" onclick=\"unhidepost_author('" + author + "');return false;\" title=\"Показать посты\"><img src=\"/css/icons/blank.gif\" border=\"0\" class=\"unhidethread spritebtn\" alt=\"Показать посты\" /></a>";
				post.after(patch);
			}
		}
	})
		...
		process_posts.forEach((post) =>
		{
			...
			if (hiddenwords_found > 0)
			{
				...
				if (document.querySelectorAll('#' + post.id + "_" + b + "hiddenbyword").length == 0)
				{
					...
					patch.classList.add("id_" + p + "_" + b);
					patch.innerHTML = "Пост " + p + " скрыт по слову &laquo;"+actual_hiddenword+"&raquo;" + ((hiddenwords_found > 1) ? " и еще " + (hiddenwords_found - 1) + " другим словам" : "") + ". Показать посты с этими словами вновь: <a class=\"extrabtns\" href=\"#\" onclick=\"unhidepost_text('"+actual_hiddenword+"');return false;\" title=\"Показать посты\"><img src=\"/css/icons/blank.gif\" border=\"0\" class=\"unhidethread spritebtn\" alt=\"Показать посты\" /></a>";
					post.after(patch);
				}
				else
				{
