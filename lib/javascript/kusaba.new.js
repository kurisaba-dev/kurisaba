...
function delandbanlinks($scope, force) {
  ...
  $scope.find('span[id^=dnb]').each(function(index,element) {
	...
	newhtml +='<a href="#" onclick="geoBlock($(this), \''+boardname+'\', '+postid+'); return false;"><img src="'+ku_boardspath+'/css/icons/blank.gif?v='+ku_cssver+'" class="geoblock spritebtn sb-r"><\/a>';
...
function geoBlock(elem, board, post)
{
	...
	$.ajax(
	{
		...
		success: function(data)
		{
			$('#cr_post').val(data.result[post]['cr_post']);
			$('#cr_file').val(data.result[post]['cr_file']);
...
function do_geoblock(is_file)
{
	...
	$.ajax(
	{
		url: ku_boardspath + '/manage_page.php?action=geoblock&field=' + field + '&board=' + board + '&post=' + post + '&data=' + data,
		success: function()
		{
...
function readyset() {
  ...
  $('<div id="geoblockform"></div>').addClass('content-background reflinkpreview qreplyform').html(
    ...
    '<input type="text" size="10" id="cr_post" name="cr_post" title="Enter ISO country codes (e.g. RU,US)"><input type="button" name="gbpost" title="Geographically block this post" value="Post" onclick="do_geoblock(false); return false;"><br />'+
    '<input type="text" size="10" id="cr_file" name="cr_file" title="Enter ISO country codes (e.g. RU,US)"><input type="button" name="gbfile" title="Geographically block this file" value="File" onclick="do_geoblock(true); return false;"><br />'+
