...
var Styles = {
  ...
  decide: function() {
    ...
    if(sc && in_array(sc, this.titles))
      ...
      this.setDefault();
      setCookie("kustyle_site",this.default,365);
  ...
  change: function(stylename) {
    ...
    if(this.hasOwnProperty('custom') && this.custom === stylename) {
      ...
      if(this.hasOwnProperty('custom'))
        this.addBypass();
      setCookie("kustyle_site",stylename,365);
...
function js_send_postbox() {
    ...
    xhr.onreadystatechange = function() {
		...
		var responseUrl = xhr.responseText.substr(12); /* Cut 'REDIRECT-TO:' */
...
function dollchanUpdaterEnabled() { return (document.getElementById('de-panel-upd-on') !== null); } /* is it needed? */
...
function kill_dollchan_incompatible_features() {
	...
	if (!localStorage['dollchan_notified']) {
		popupMessageOn('Обнаружен куклоскрипт!<br>Функции, несовместимые с куклоскриптом, во время его работы автоматически выключены.<div style="font-weight: normal; font-size: 11px;"><br />Кликни, чтобы закрыть</div>');
...
var Settings = {
  ...
  sfwMode: function(changed) {
    ...
    if (changed != null) {
      ...
      if(enabled) {
        addCSSRules(".thumb { opacity: 0.05;}", "sfwMode0");
        addCSSRules(".thumb:hover { opacity: 1;}", "sfwMode1");
      } else if(changed) {
...
function readyset() {
  ...
  var userExpanded = false;
	...
	$('.sect-exr').click(function() {
	  var toexpandId = 'ms-' + $(this).data('toexpand');
	  if($('#' + toexpandId).is(':visible') && userExpanded === true) {
		userExpanded = false;
		menu_show('_off_');
	  } else {
  ...
  if(!isTouchDevice) {
    ...
    $('.sect-exr').mouseenter(function() { if (userExpanded !== false) return; menu_show('ms-'+$(this).data('toexpand')); });
    $('#overlay_menu').mouseleave(function() { if (userExpanded !== false) return; menu_show('_off_'); });
  ...
  if(react_ena && typeof io !== 'undefined') {
    ...
    addCSSRules('.de-ppanel-cnt:after {display: none}', 'dePostCounterHider');
...
var dcxt = {
  ...
  openSettings: function() {
    ...
    addCSSRules('#de-content-cfg {opacity: 0!important}', 'dcxt');
  ...
  performTasks: function() {
    if(!this.enabled && $('#de-btn-settings').length) {
      this.enabled = true;
      document.getElementById('gotothread').checked = true;
...
function admin_check_reports() {
	...
	xhr.onreadystatechange = function() {
		if (xhr.readyState != 4)
			return;
		if (xhr.status != 200) {
			popupMessageOn('Кажется, что-то пошло не так! XMLHttpRequest::status = ' + xhr.status + '<div style="font-weight: normal; font-size: 11px;"><br />Кликни, чтобы закрыть</div>');
			return;
		}
		...
		if (count == "") {
			popupMessageOn('Кажется, что-то пошло не так!<br />Ошибка получения списка репортов. (есть ли доступ в админку?)<div style="font-weight: normal; font-size: 11px;"><br />Кликни, чтобы закрыть</div>');
			return;
		}
		...
		popupMessageOn('Загляни в админку.<br />Новых репортов: ' + count + '<div style="font-weight: normal; font-size: 11px;"><br />Кликни, чтобы закрыть</div>');
	};
}
