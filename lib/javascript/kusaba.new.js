...
var Settings = {
  ...
  fullfeed: function(changed) {
    ...
    if (enabled)
    {
      ...
      if($("iframe", window.parent.document).each((x,f) => {$(".feedlink", f.contentDocument).attr("href",$(".feedlink", f.contentDocument).attr("href") + "?fullfeed")}).length == 0)
      {
        $(".feedlink").attr("href",$(".feedlink").attr("href") + "?fullfeed");
      }
    } else {
      if($("iframe", window.parent.document).each((x,f) => {$(".feedlink", f.contentDocument).attr("href",$(".feedlink", f.contentDocument).attr("href").replace("?fullfeed",""))}) == 0)
