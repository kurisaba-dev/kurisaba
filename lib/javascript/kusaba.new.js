...
var Settings = {
  ...
  fullfeed: function(changed) {
    ...
    if (enabled)
    {
      ...
      if($("iframe", window.parent.document).each((x,f) => {if($(".feedlink", f.contentDocument)) $(".feedlink", f.contentDocument).attr("href",$(".feedlink", f.contentDocument).attr("href") + "?fullfeed");}).length == 0)
      {
        $(".feedlink").attr("href",$(".feedlink").attr("href") + "?fullfeed");
      }
    } else {
      if($("iframe", window.parent.document).each((x,f) => {if($(".feedlink", f.contentDocument)) $(".feedlink", f.contentDocument).attr("href",$(".feedlink", f.contentDocument).attr("href").replace("?fullfeed",""));}) == 0)
    ...
    if(changed)
    {
       if(window.frames.length == 2)
         if(window.frames[1].location.href.includes("single.php"))
           window.frames[1].location.href = ku_boardspath + "/single.php" + (enabled ? "?fullfeed" : "");
         else
