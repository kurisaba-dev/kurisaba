		...
		if (xhr.readyState == 4)
		{
			if (xhr.status == 200)
			{
				...
				else
				{
					...
					popupMessageTexted(msgdel3 + post.value + ':', xhr.responseText);
				}
			}
			else
			{
		...
		xhr.onreadystatechange = function()
		{
			if (xhr.readyState == 4)
			{
				if (xhr.status == 200)
				{
					...
					{
						popupMessageTexted(xhr.responseText, '');
					}
				}
				else
				{
...
function js_send_postbox() {
	...
	if(form.deletepostp && form.deletepostp.value != '') {
		...
		xhr.onreadystatechange = function() {
			...
			if (xhr.responseText.search('REDIRECT-TO:') != 0) {
				popupMessageTexted('Произошла ошибка при удалении поста ' + form.deletepostp.value + ':', xhr.responseText);
				return;
			}
		...
		if (xhr.readyState == 4)
		{
			...
			if (xhr.status == 200)
			{
				...
				else
				{
					...
					if (!result.notify && !automatic) popupMessageTmp(_.noNewPosts);
...
function popupMessageHttpError(code, text) {
	...
	popupMessageTexted('Кажется, что-то пошло не так!', text);
}
