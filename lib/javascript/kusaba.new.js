...
function delandbanlinks($scope, force) {
  ...
  $scope.find('span[id^=dnb]').each(function(index,element) {
	...
	newhtml +='<a href="#" onclick="(e) => { geoBlock(e, \''+boardname+'\', '+postid+'); }"><img src="'+ku_boardspath+'/css/icons/blank.gif?v='+ku_cssver+'" class="geoblock spritebtn sb-r"><\/a>';
...
function geoBlock(e, board, post)
{
	$('#cr_board').val(board);
	$('#cr_postid').val(post);
	$.ajax(
	{
		...
		success: function(data)
		{
			...
			$('#geoblockform').css({top: e.clientY, left: e.clientX}).fadeIn('fast');
		},
		error: function()
		{
			$('#cr_post').val('?');
...
function readyset() {
  ...
  $('<div id="geoblockform"></div>').addClass('content-background reflinkpreview qreplyform').html(
    '<form ><input type="hidden" name="cr_board"><input type="hidden" name="cr_postid"><div style="display:inline-block">'+
