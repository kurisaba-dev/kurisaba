...
function delandbanlinks($scope, force) {
  ...
  $scope.find('span[id^=dnb]').each(function(index,element) {
	...
	newhtml +='<a href="' + ku_cgipath + '/manage_page.php?action=bans&ban_img_by_post=true&banboard=' + boardname + '&banpost=' + postid + '" title="'+_.banfile+'" onclick="return confirm(\''+_.banfile+'?\');"><img src="'+ku_boardspath+'/css/icons/blank.gif" class="banfile spritebtn sb-c"><\/a>';
	
	newhtml +='<a href="#" onclick="(e) => { geoBlock(e, \''+boardname+'\', '+postid+'); }"><img src="'+ku_boardspath+'/css/icons/blank.gif" class="geoblock spritebtn sb-r"><\/a>';
	...
	hiddenposts.forEach((post_to_hide) =>
	{
		...
		document.querySelectorAll('#reply' + p).forEach((post) =>
		{
			...
			if(post_b == b && !post.classList.contains("hiddenbypostid"))
			{
				...
				patch.innerHTML = "Пост "+b+"/"+p+" скрыт по номеру. Показать вновь: <a class=\"extrabtns\" href=\"#\" onclick=\"unhidepost_num('"+b+"',"+p+");return false;\" title=\"Показать этот пост\"><img src=\"/css/icons/blank.gif\" border=\"0\" class=\"unhidethread spritebtn\" alt=\"Показать этот пост\" /></a>";
				post.after(patch);
			}
		});
	});
					...
					if (p != "")
					{
						document.querySelectorAll('#reply' + p).forEach((post) =>
						{
							...
							if(post_b == b && !post.classList.contains("hiddenbythreadid"))
							{
								...
								patch.classList.add("id_" + p + "_" + b);
								patch.innerHTML = "Тред "+b+"/"+p+" скрыт по номеру. Показать вновь: <a class=\"extrabtns\" href=\"#\" onclick=\"unhidethread_num('"+b+"',"+p+");return false;\" title=\"Показать этот тред\"><img src=\"/css/icons/blank.gif\" border=\"0\" class=\"unhidethread spritebtn\" alt=\"Показать этот тред\" /></a>";
								post.after(patch);
							}
						});
						if (typeof(isfeed) != "undefined" && isfeed)
						{
							document.querySelectorAll('.reply_parent' + p).forEach((post) =>
							{
								...
								if(post_b == b && !post.classList.contains("hiddenbyparentid"))
								{
									...
									patch.innerHTML = "Пост "+b+"/"+a+" скрыт, так как тред "+b+"/"+p+" скрыт по номеру. Показать вновь: <a class=\"extrabtns\" href=\"#\" onclick=\"unhidethread_num('"+b+"',"+p+");return false;\" title=\"Показать этот тред\"><img src=\"/css/icons/blank.gif\" border=\"0\" class=\"unhidethread spritebtn\" alt=\"Показать этот тред\" /></a>";
									post.after(patch);
								}
							});
						}
	...
	process_posts.forEach((post) =>
	{
		...
		else
		{
			if (in_array(author, hiddenauthors))
			{
				...
				patch.classList.add("id_" + p + "_" + b);
				patch.innerHTML = "Пост " + b + "/" + p + ", написанный " + author + ", скрыт по автору. Показать его посты вновь: <a class=\"extrabtns\" href=\"#\" onclick=\"unhidepost_author('" + author + "');return false;\" title=\"Показать посты\"><img src=\"/css/icons/blank.gif\" border=\"0\" class=\"unhidethread spritebtn\" alt=\"Показать посты\" /></a>";
				post.after(patch);
			}
		}
	})
...
function readyset() {
  ...
  $('<div id="geoblockform"></div>').addClass('content-background reflinkpreview qreplyform').html(
    ...
    '</div><a href="#" onclick="javascript:$(\'#geoblockform\').hide();return false;" title="'+_.close+'"><img style="vertical-align: top;" src="'+ku_boardspath+'/css/icons/blank.gif" border="0" class="closebox spritebtn"></a></form>'
  ).hide().appendTo('body');
