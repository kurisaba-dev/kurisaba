...
var notificationIcon =  ku_boardspath + "/images/sg-logo-G-128.png";
...
function setLocalYou(elemArr) {
	...
	for (var elemId = 0; elemId < elemArr.length; elemId++) {
		...
		for (var i = 0; i < elems.length; i++) {
			...
			var targetInstall = elem.querySelector("span.datetime").previousElementSibling.nextSibling;
			if (targetInstall.previousElementSibling.className == "youindicator")
				continue;
			var insElem = document.createElement("span");
			insElem.className = "youindicator";
...
function tasksAfterModify(newly_added) {
	...
	$('.unkfunc').filter(function(){
		return this.innerHTML.substr(0,8) == '&gt;&gt;';
	}).toggleClass('unkfunc unkfunc2');
}
...
function expand_post(element, board, threadid, postid)
{
	...
	xhr.onreadystatechange = function() {
		...
		textnode.innerHTML = retData.result[postid].text.replace(new RegExp('\\\\', 'g'), '');
		tasksAfterModify("");
	};
	return false;
}
...
function post_preview(e)
{
	...
	xhr.onreadystatechange = function() {
		...
		if (xhr.status != 200) {
			popupMessageHttpError(xhr.status, xhr.statusText);
			close_post_preview(true);
			close_post_preview(false);
			return;
		}
...
function myPostsAdd(postid) {
	postid = parseInt(postid);
	if (!(postid > 0))
		return;
	myPostsUpdate();
	window.myPosts[postid] = 1;
...
function myPostsCheck(postid) {
	postid = parseInt(postid);
	if (!window.myPosts)
		myPostsUpdate();
	return window.myPosts[postid] ? true : false;
}
